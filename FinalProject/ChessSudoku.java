...
public class ChessSudoku
{
	...
	//helper function
	private  boolean Backtracking(int[][] grid, int row, int col)
	{
	...
	private static boolean rule(int[][]grid,int row,int col,int num)
	{
		for (int x=0;x < 8;x++)
		{
			if (grid[row][x]== num)
			...
			//--------------------------------
			//same num can't appear in 3x3

			int unitRow = row-row%3;
			int unitCol = col-col%3;

			for (int i=0;i < 3;i++)
			{
				for (int j=0;j<3;j++)
				{
					if (grid[i+unitRow][j+unitCol]==num)
					{
						return false;
					}
				}
